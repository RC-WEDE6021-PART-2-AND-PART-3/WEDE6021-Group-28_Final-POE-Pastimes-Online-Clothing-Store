...
class Clothing {
     ...
     * @return bool Success
     */
    public function update($clothingID, $data) {
        try {
            $stmt = $this->conn->prepare(
                "UPDATE tblClothes SET title = ?, brand = ?, category = ?, size = ?, itemCondition = ?, price = ?, description = ?, imagePath = ?, suggestedPrice = ?, co2Saved = ?, waterSaved = ?, status = ? WHERE clothingID = ?"
            );
