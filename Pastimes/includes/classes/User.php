...
class User {
    ...
    /**
     * Admin update user (includes role and status changes)
     * @param int $userID
     * @param array $data
     * @return bool
     */
    public function adminUpdate($userID, $data) {
        try {
            $stmt = $this->conn->prepare(
                "UPDATE tblUser SET fullName = ?, email = ?, username = ?, role = ?, status = ?, phone = ?, address = ?, city = ?, postalCode = ? WHERE userID = ?"
            );
            ...
            $ok = $stmt->execute();
            if ($ok && function_exists('appendPlainDataFile')) {
                $line = 'admin_update' . "\t" . (int)$userID . "\t" . $data['fullName'] . "\t" . $data['email'] . "\t" . $data['username'] . "\t" . $data['role'] . "\t" . $data['status'] . "\t" . date('c');
                appendPlainDataFile('userData.txt', $line);
            }
            return $ok;
        } catch (Exception $e) {
