 ...
 */
function image_url($filename, $category = null) {
    ...
    if (stripos($filename, 'images/') !== false || stripos($filename, 'image/') !== false) {
        ...
        // Resolve filesystem path for existence check
        $fsPath = __DIR__ . '/../' . ltrim($sub, '/');
        if (!file_exists($fsPath)) {
            // Attempt case-insensitive match in images directory for common filename variations
            $imagesDir = __DIR__ . '/../images/';
            if (is_dir($imagesDir)) {
                $requested = ltrim($sub, '/');
                $requestedName = pathinfo($requested, PATHINFO_FILENAME);
                $requestedNorm = preg_replace('/[^a-z0-9]/i', '', strtolower($requestedName));
                foreach (scandir($imagesDir) as $f) {
                    if ($f === '.' || $f === '..') continue;
                    $candName = pathinfo($f, PATHINFO_FILENAME);
                    $candNorm = preg_replace('/[^a-z0-9]/i', '', strtolower($candName));
                    // Exact match
                    if ($candNorm === $requestedNorm) return '/pastimes/images/' . rawurlencode($f);
                    // Substring match (requested part of candidate or vice versa)
                    if ($candNorm !== '' && (strpos($candNorm, $requestedNorm) !== false || strpos($requestedNorm, $candNorm) !== false)) return '/pastimes/images/' . rawurlencode($f);
                    // Fuzzy match using levenshtein with adaptive threshold
                    $dist = levenshtein($candNorm, $requestedNorm);
                    $threshold = max(2, (int)round(strlen($requestedNorm) * 0.15));
                    if ($dist <= $threshold) return '/pastimes/images/' . rawurlencode($f);
                }
            }

            // If file is missing, return a local category-based SVG placeholder when available
            if (!empty($category)) {
                $slug = preg_replace('/[^a-z0-9]+/i', '-', strtolower($category));
                $placeholder = '/pastimes/images/placeholder-' . $slug . '.svg';
                $placeholderFs = __DIR__ . '/../images/placeholder-' . $slug . '.svg';
                if (file_exists($placeholderFs)) return $placeholder;
            }
            // Generic placeholder
            return '/pastimes/images/placeholder-generic.svg';
        }
    ...
    $web = '/pastimes/images/' . ltrim($filename, '/');
    $fs = __DIR__ . '/../images/' . ltrim($filename, '/');
    if (!file_exists($fs)) {
        // Try to find a case-insensitive match for the filename inside images/
        $imagesDir = __DIR__ . '/../images/';
        if (is_dir($imagesDir)) {
            $requestedName = pathinfo($filename, PATHINFO_FILENAME);
            $requestedNorm = preg_replace('/[^a-z0-9]/i', '', strtolower($requestedName));
            foreach (scandir($imagesDir) as $f) {
                if ($f === '.' || $f === '..') continue;
                $candName = pathinfo($f, PATHINFO_FILENAME);
