...
// Get orders
$stmt = $conn->prepare("SELECT o.*, c.title, c.sellerID, u.fullName as buyerName, s.fullName as sellerName FROM tblOrder o JOIN tblClothes c ON o.clothingID = c.clothingID JOIN tblUser u ON o.buyerID = u.userID LEFT JOIN tblUser s ON c.sellerID = s.userID ORDER BY o.orderDate DESC");
$stmt->execute();
    ...
    <tbody>
        <?php foreach ($orders as $order): ?>
            <tr>
                ...
                <td>
                    <a href="compose_message.php?receiver=<?php echo $order['buyerID']; ?>&clothingID=<?php echo $order['clothingID']; ?>" class="btn btn-primary" style="padding:0.4rem 0.6rem; margin-right:6px;">Message Buyer</a>
                    <a href="compose_message.php?receiver=<?php echo $order['sellerID']; ?>&clothingID=<?php echo $order['clothingID']; ?>" class="btn btn-secondary" style="padding:0.4rem 0.6rem;">Message Seller</a>
                </td>
            </tr>
        <?php endforeach; ?>
    </tbody>
