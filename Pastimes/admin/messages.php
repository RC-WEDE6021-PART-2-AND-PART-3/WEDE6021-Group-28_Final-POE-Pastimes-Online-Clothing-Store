...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($subject && $messageBody) {
        ...
        // Send messages
        // Determine or create a system user to act as sender for admin broadcasts
        ...
        if ($sstmt) {
            ...
            $row = $res->fetch_assoc();
            if ($row) $senderID = (int)$row['userID'];
            $sstmt->close();
        }
        if (!$senderID) {
        ...
        foreach ($recipients as $receiverID) {
            try {
                ...
                $stmt->bind_param('iiss', $senderID, $receiverID, $subject, $messageBody);
                if ($stmt->execute()) {
