            ...
            if (!in_array($fileType, $allowedTypes)) {
                ...
                if (move_uploaded_file($_FILES['image']['tmp_name'], $uploadPath)) {
                    ...
                    try {
                        ...
                        // Types: i = int (userID), s = string (title, brand, category, size, itemCondition, description, imagePath), d = double (price, co2Saved), i = int (waterSaved)
                        $stmt->bind_param('isssssdssdi', $userID, $title, $brand, $category, $size, $condition, $price, $description, $uploadPath, $co2Saved, $waterSaved);
                        $stmt->execute();
...
<body>
    ...
    <main class="container" style="max-width: 1200px; margin: 0 auto; padding: 2rem 1rem;">
        ...
        <?php else: ?>
            ...
            <form id="sellForm" method="POST" enctype="multipart/form-data" class="sell-form">
                <div class="form-group">
                    <label for="itemImage">ITEM PHOTO</label>
                    <div id="imageDropZone" class="image-upload" role="button" tabindex="0">
                        <div class="drop-zone-content" style="display:flex;flex-direction:column;align-items:center;">
                            ...
                            <button type="button" id="browseFiles" style="background: #1e40af; color: white; border: none; padding: 0.5rem 1rem; border-radius: 4px; cursor: pointer; margin-top: 0.5rem;">Click to browse files</button>
                            <p style="margin: 0.5rem 0; color: #6b7280; font-size: 0.85rem;">JPG, PNG, WebP — max 5MB</p>
                        </div>
                    </div>
                    <input type="file" id="itemImage" name="image" accept="image/*" required style="display:none;">
    ...
    <script>
        ...
        const itemInput = document.getElementById('itemImage');
        if (itemInput) {
            itemInput.addEventListener('change', function(e) {
                ...
                if (dropContent) {
                    dropContent.innerHTML = `<i class="fas fa-check-circle" style="color: #10b981;"></i><p style="margin: 0.5rem 0; font-weight: 600;">${fileName}</p><p style="margin: 0; color: #6b7280; font-size: 0.9rem;">Ready to upload</p>`;
                }
            });
        }
    </script>

    <?php $pageScript = 'sell.js'; include 'includes/footer.php'; ?>
</body>
</html>
